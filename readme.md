...
1) Название теста должно соответствовать названию REST - контроллера с добавлением приставки `Test`,  например, если есть *ResourceAnswerController*, тест будет называться *TestResourceAnswerController* , тестируются в нём все *api* из контроллера.
2) В пакете `dataset/expected` лежат все `dataset` для сравнения получившейся базы данных после завершения теста, данные `dataset` не используются для обычной загрузки данных для тестов, а используются только для операций **DELETE**, **UPDATE**, **PERSIST**.
3) Все сущности, описанные в `dataset `для загрузки тестовых данных, начинаются с *id = 100* (никакого отношения к dataset из expected не имеют).
4) Все тестовые классы должны наследоваться от *абстрактного класса*, где описана все конфигурация тестов.
5) **НЕЛЬЗЯ ИЗМЕНЯТЬ НАПИСАННЫЕ DATASET!!!** Если хотите добавить данные, то необходимо создать подпакет для вашего конкретного теста и написать свои `dataset` для этого теста.
6) На каждый класс тестов должны быть написаны отдельные `dataset`. В случае, где их нужно изменить, применяется п. 5. Например, если мы тестируем *ResourceAnswerController*, обращаемся к подпакету `dataset`/[название пакета], в котором находятся все `dataset`, нужные для тестирования этого контроллера.
