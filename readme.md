...
```
Создал файл **database.dtd** с конфигурацией наших таблиц (вспомогательный файл, для работы с dataset-файлами, его наличие не обязательно). Файлы DTD - это простые текстовые файлы, используемые разработчиками, такие файлы дают дополнительную информацию о различных атрибутах и элементах, которые используются в коде документов, написанных с помощью XML, HTML, XHTML или SGML. Назначение файлов DTD - проверка корректности распознавания файлов, содержащих код на одном из указанных выше языков.
