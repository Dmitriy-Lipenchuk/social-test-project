...
Для того чтобы можно было использовать разные настройки (например, работа с разными базами данных) сделаны **properties-файлы**
...
```
Для быстрого переключения конфигурации с local на dev необходимо в файле application.properties 
поменять значение поля spring.profiles.active с local на dev
```
