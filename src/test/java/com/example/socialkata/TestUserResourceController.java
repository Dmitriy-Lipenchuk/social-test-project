...
//        @Test
//        @DataSet(value = "user.xml", cleanAfter = true)
//        public void getUserToIdFailure() throws Exception {
//
//            mvc.perform(MockMvcRequestBuilders.get("/api/user/{userId}", 55))
//                      .andExpect  (MockMvcResultMatchers.status().isBadRequest());
//                    .andExpect(jsonPath("$.userId",is(55)))
//                    .andExpect(jsonPath("$.firstName").value("Pep"))
//                    .andExpect  (jsonPath("$.lastName").value("Flow"))
//                    .andExpect  (jsonPath("$.dateOfBirth").value("1901-07-28"))
//                    .andExpect  (jsonPath("$.education").value("Master degree"))
...
//                    .andExpect  (jsonPath("$.status").value("Chill"))
//                    .andExpect  (jsonPath("$.activeName").value("active"))
//                    .andExpect  (MockMvcResultMatchers.status().reason(is("User with this id was not found in the database.")));
